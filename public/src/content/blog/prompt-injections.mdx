...
description: "Prompt injection is the most important security problem in agent development. It happens when an attacker controls part of the agent's context and uses that to make it do something the user did not ask for. The attack surface includes any untrusted content the agent sees: webpages, emails, issue threads, shared docs, tool outputs, MCP metadata, memory lookups, and artifacts from other agents. The damage comes from what the agent does with that content: sending information to a third party, following a link, using a tool, writing to memory, or handing off to another agent. Defending against prompt injection requires treating it as an infrastructure problem with trust boundaries, scoped credentials, and auditable tool chains."
date: 2026-03-15
...
[^1]: OpenAI, Operator system card, January 23, 2025. Agent Security Bench, ICLR 2025 poster published January 22, 2025.

[^2]: OpenAI, Operator system card. The same source also reports 46 benign-screen flags out of 13,704 and 92% confirmation recall across 607 risky tasks.

[^3]: OpenAI, Deep Research system card, February 25, 2025.

[^4]: OpenAI, New tools for building agents, March 11, 2025.

[^5]: OpenAI, Operator system card API update, March 11, 2025.

[^6]: Anthropic, Mitigating the risk of prompt injections in browser use, November 24, 2025.

[^7]: Microsoft MSRC, How Microsoft defends against indirect prompt injection attacks, July 29, 2025.

[^8]: OpenAI, Designing AI agents to resist prompt injection, March 11, 2026.

[^9]: Invariant Labs, GitHub MCP exploited: Accessing private repositories via MCP, May 26, 2025.

[^10]: Microsoft MSRC, How Microsoft defends against indirect prompt injection attacks.

[^11]: Invariant Labs, MCP Security Notification: Tool Poisoning Attacks, April 1, 2025.

[^12]: Model Context Protocol specification, March 26, 2025 and June 18, 2025.

[^13]: Invariant Labs, GitHub MCP exploited: Accessing private repositories via MCP.

[^14]: Invariant Labs notes that many users already opt for broad approval policies in agent tooling, which weakens the value of per-call confirmations.

[^15]: Sunil et al., Memory Poisoning Attack and Defense on Memory Based LLM-Agents, arXiv:2601.05504, January 2026.

[^16]: The same paper cites earlier MINJA results above 95% injection success and 70% attack success under idealized conditions while showing weaker attacks under more realistic memory states.

[^17]: Sunil et al., Memory Poisoning Attack and Defense on Memory Based LLM-Agents.

[^18]: Google, Announcing the Agent2Agent Protocol, April 9, 2025.

[^19]: Google's A2A description highlights agent cards, task objects, messages, and artifacts as first-class protocol objects.

[^20]: Google Cloud, Announcing a complete developer toolkit for scaling A2A agents on Google Cloud, August 1, 2025.

[^21]: Google Workspace Admin Help, Indirect prompt injections and Google's layered defense strategy for Gemini.

[^22]: OpenAI, Understanding prompt injections: a frontier security challenge, November 7, 2025, and Continuously hardening ChatGPT Atlas against prompt injection attacks, December 22, 2025.

[^23]: Anthropic, Mitigating the risk of prompt injections in browser use.

[^24]: Invariant Labs proposes one-repo-per-session style controls to prevent cross-repository toxic flows in MCP-driven coding workflows.

[^25]: OpenAI, Keeping your data safe when an AI agent clicks a link, January 28, 2026.