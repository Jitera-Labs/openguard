...
class AsyncEventEmitter:
    ...
    async def once(self, event_name: str, listener: Callable):
        ...
        logger.debug(f"ONCE: {listener.__name__} for '{event_name}'")  # type: ignore
    ...
    async def off(self, event_name: str, listener: Callable):
        async with self._lock:
            ...
            if removed:
                logger.debug(f"OFF: {listener.__name__} from '{event_name}'")  # type: ignore
            else:
                logger.warning(f"OFF_NOT_FOUND: {listener.__name__} for '{event_name}'")  # type: ignore

    async def remove_all_listeners(self, event_name: str | None = None):
        async with self._lock:
            if event_name:
    ...
    async def emit(self, event_name: str, *args, **kwargs):
        ...
        logger.info(
            f"EMIT: '{event_name}' "
            f"(Listeners: {len(listeners_to_call) + len(once_listeners_to_call)})"
        )

        all_listeners = listeners_to_call + once_listeners_to_call
        tasks = [
            self._call_listener(listener, event_name, *args, **kwargs) for listener in all_listeners
        ]
        await asyncio.gather(*tasks, return_exceptions=False)

    async def _call_listener(self, listener: Callable, event_name: str, *args, **kwargs):
        try:
            await listener(*args, **kwargs)
        except Exception as e:
            logger.error(
                f"LISTENER_ERROR: {listener.__name__} on '{event_name}': {e}",
                exc_info=False,  # type: ignore
            )
