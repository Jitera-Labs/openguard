...
class StrList(List[str]):
    ...
    def from_string(cls, value: str) -> "StrList":
        return (cls(item.strip() for item in value.split(";")
                    if item.strip()) if value.strip() else cls())
...
class BoolList(List[bool]):
    ...
    def from_string(cls, value: str) -> "BoolList":
        return (cls(item.strip().lower() == "true" for item in value.split(";")
                    if item.strip()) if value.strip() else cls())
...
class Config(Generic[T]):
    ...
    def _resolve_single(self) -> T:
        ...
        if config_key in PERSISTENT_CONFIG:
            ...
            if issubclass(self.type,
                          (ConfigDict, dict)) and isinstance(val, dict):
    ...
    def _resolve_wildcard(self) -> List[T]:
        ...
        matching_vars = [(key, value) for key, value in os.environ.items()
                         if key.startswith(prefix)]

        env_values = [
            self._convert_value(value) for _, value in sorted(matching_vars)
        ]
    ...
    def _convert_value(self, value: str) -> T:
        if issubclass(self.type,
                      (StrList, IntList, FloatList, BoolList, ConfigDict)):
...
OPENGUARD_OPENAI_URLS = Config[str](
    ...
    description=("Downstream OpenAI-compatible API URLs. "
...
OPENGUARD_ANTHROPIC_URLS = Config[str](
    ...
    type=str,
    default="",
    description=("Downstream Anthropic Chat API URLs. "
...
OPENGUARD_ANTHROPIC_KEYS = Config[str](
    ...
    type=str,
    default="",
    description=("API keys for downstream Anthropic APIs. "
...
OPENGUARD_API_KEY = Config[str](
    ...
    default="",
    description=
    "API key required to access this OpenGuard proxy. Leave empty to disable auth.",
)
...
MODEL_FILTER = Config[ConfigDict](
    ...
    default="",
    description=
    "Filter to apply to downstream models list (Hasura-style filter).",
)
...
STATUS_STYLE = Config[str](
    ...
    description=
    "Style for status messages (md:codeblock, md:h1, md:h2, md:h3, plain, none)",
)
