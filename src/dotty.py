...
def _detect_callable_type(fn: Any) -> str:
  ...
  if inspect.isfunction(fn) or inspect.isbuiltin(fn):
    ...
    if (inspect.isfunction(fn) and
        hasattr(fn, '__qualname__') and
        '.' in fn.__qualname__ and
        not hasattr(fn, '__self__')):
...
def call(obj: Any, path: DottyPath, args: Optional[List[Any]] = None, kwargs: Optional[dict] = None) -> Any:
  """
...
def get(obj: Any, path: DottyPath, default: Any = None) -> Any:
  """
    Gets the value at path of object. If the resolved value is undefined, the default value is returned.
    Works with nested dictionaries, lists, class attributes, and properties.
...
def update(obj: Any, path: DottyPath, value: Any) -> Any:
  """
    Updates the dictionaty at the path of object. If the leaf value is not a dictionary - an error is raised.
...
def pop(obj: Any, path: DottyPath, default: Any = None) -> Any:
  """
    Pops the value at path of object. If the resolved value is undefined, the default value is returned.
    Works with nested dictionaries, lists, class attributes, and properties.
...
def delete(obj: Any, path: DottyPath) -> Any:
  ...
  if isinstance(root, dict) and last_part in root:
    del root[last_part]
  elif isinstance(root, list) and isinstance(last_part, int) and -len(
    root) <= last_part < len(root):
    del root[last_part]
  elif hasattr(root, last_part):
    delattr(root, last_part)
  else:
    try:
      del root[last_part]
    except (KeyError, TypeError, IndexError):
