from typing import TYPE_CHECKING, List, Dict, Any, TypeVar, Union
import random
import re
...
def filter_array_elements(items: List[T], filter_obj: Dict[str,
                                                           Any]) -> List[T]:
  """
...
def find_array_element(items: List[T], filter_obj: Dict[str,
                                                        Any]) -> Union[T, None]:
  """
...
def match_filter(context: Any, filter_obj: Dict[str, Any]):
  ...
  if '_and' in filter_obj:
    return all(
      match_filter(context, sub_filter) for sub_filter in filter_obj['_and']
    )

  if '_or' in filter_obj:
    return any(
      match_filter(context, sub_filter) for sub_filter in filter_obj['_or']
    )
...
def apply_operator(field_value, operator, expected_value):
  ...
  if operator == '_contains':
    if isinstance(field_value, dict) and isinstance(expected_value, dict):
      ...
      return all(
        k in field_value and field_value[k] == v
        for k, v in expected_value.items()
      )
    elif isinstance(field_value, list):
      # Check if expected_value is in the list
      return expected_value in field_value
    elif isinstance(expected_value,
                    list) and not isinstance(field_value, (dict, list)):
